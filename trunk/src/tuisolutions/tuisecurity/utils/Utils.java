...
import tuisolutions.tuisecurity.controllers.Mailer;
import tuisolutions.tuisecurity.controllers.FileManager;
...
public class Utils {
	 ...
	 * @param pathFile
	 */
	public static void sendEmail(Context context, String pathFile,
			boolean deleteIfSuccess) {
		if (checkInternetConnection(context)) {
			// Get all file retrived and send to email seted up
			Mailer mail = new Mailer(context,
					PreferencesUtils.getFromEmail(context),
					PreferencesUtils.getFromEmailPassword(context));
	...
	public static boolean checkInternetConnection(Context context) {
		countToEnable = 0;
		while (!PhoneUtils.isDataEnable(context) && countToEnable < 60000) {
			if (countToEnable == 0) {
				// Enable only in firstime
				PhoneUtils.setMobileDataEnabled(context, true);
			}
			countToEnable ++;
		}
	...
	public static String convertToASCII(String text) {
		if (text != null) {
			return Normalizer.normalize(text, java.text.Normalizer.Form.NFD)
	...
	public static String getDateNow() {
		Date time = new Date();
		return DateFormat.format("dd.MM.yy-hh.mm.ss", time.getTime())
				.toString();
	}
	...
	public static String md5(String s) {
		...
		try {
			...
			for (int i = 0; i < messageDigest.length; i++) {
				sb.append(Integer.toString((messageDigest[i] & 0xff) + 0x100,
						16).substring(1));
			}
			return sb.toString();
